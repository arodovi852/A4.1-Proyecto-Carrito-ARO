...
export class PerfilUsuarioComponent implements OnInit {
  ...
  calcularEdad(): number {
    const fecha = this.informacionPersonal.get('fechaNacimiento')?.value;
    if (!fecha) return 0;
  ...
  generarResumen(): string {
    ...
    const d = this.direcciones.length;

    return `${info.nombre} ${info.apellido}, ${edad} años, ${d} dirección(es) registrada(s).`;
  }
